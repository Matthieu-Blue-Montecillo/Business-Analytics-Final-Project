...
df = pd.read_csv(r'C:\project\eda\myenv\data\Group_7_Dataset.csv')
...
<html>
    <head>
        ...
        {%css%}
        <style>
            .kpi-tooltip { visibility: hidden; }
            *:hover > .kpi-tooltip { visibility: visible !important; }
        </style>
    </head>
    <body>
        {%app_entry%}
...
def update_kpis(store):
    ...
    def kpi_card(label, value, tooltip=None):
        if tooltip:
            value_element = html.Div(style={'position': 'relative', 'display': 'inline-block'}, children=[
                html.H2(value, style={
            ...
            value_element = html.H2(value, style={
                'margin': '0', 'fontSize': '24px',
                'color': ACCENT, 'fontWeight': 'bold',
            })
    ...
    if store == 'All Stores':
        top = df.groupby('store_location')['Total_Bill'].sum().idxmax()
        fourth_card = kpi_card('Top Store', top)
    else:
        store_rev = d.groupby('product_category')['Total_Bill'].sum()
        store_share = (store_rev / store_rev.sum() * 100)
        chain_share = (df.groupby('product_category')['Total_Bill'].sum() /
               df['Total_Bill'].sum() * 100)
        diff = store_share - chain_share
        over_index = diff.idxmax()
        diff_val = diff.max()
        tooltip_text = f'{over_index} makes up {store_share[over_index]:.1f}% of {store} revenue vs {chain_share[over_index]:.1f}% chain average'
        fourth_card = kpi_card('Signature Category', over_index, tooltip=tooltip_text)
...
def update_heatmap(store):
    ...
    pivot_pct = pivot.div(pivot.sum(axis=0), axis=1).round(3) * 100
